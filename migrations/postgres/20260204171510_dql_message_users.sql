...
COMMENT ON COLUMN dql_message.object_index IS 'index in message of list objects';
COMMENT ON COLUMN dql_message.payload IS 'content of one object in message';
COMMENT ON COLUMN dql_message.last_attempt_time IS 'time of last attempt object processing';
-- +goose StatementEnd

-- +goose Down
-- +goose StatementBegin
DROP TABLE IF EXISTS dql_message;
-- +goose StatementEnd
